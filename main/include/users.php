...
class Users
{
	protected static  $tblname = "user_access";
	...
	function find_Users($id = "", $name = "")
	{
		global $mydb;
		$mydb->setQuery("SELECT * FROM " . self::$tblname . " 
			WHERE UserID = {$id} OR Lastname = '{$name}'");
	...
	{
		global $mydb;
		$mydb->setQuery("SELECT * FROM `user_access` WHERE `Email` = '" . $email . "' and `Password` = '" . $h_pass . "'");
		$cur = $mydb->executeQuery();
		if ($cur == false) {
			//die(mysql_error());
		}
